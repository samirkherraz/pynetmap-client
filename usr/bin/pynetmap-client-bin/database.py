...
class Database:
    ...
    def get(self, table, key, attr=None):
        # try:
        #     return self.tables[table].get(key)
        # except:
            return self.api.get(table, key, attr)
    ...
    def find_by_attr(self,  value, key=None):
        return self.api.find_attr(value, key)
        # out = []
    ...
    def find_by_schema(self, schema):
        return self.api.find_attr(schema,KEY_TYPE )
        # out = []
        # for key in self.get_table("base"):
